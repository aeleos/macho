...
void box_created(long state) { boxes |= state; }
void box_destroyed(long state) { boxes &= ~state; }
bool has_box(long state) { return (boxes & state) == boxes; }
...
private:
  ...
  void entry() {
    TOP::box().entry(STATE_A);
    box().data = 42;
  }
  void exit() {
    TOP::box().exit(STATE_A);
    assert(box().data == 42);
  }
  void init() { TOP::box().init(STATE_A); }
};

SUBSTATE(StateAA, StateA){STATE(StateAA)

                              private :
                                  // Testing proper box handling here
                                  void entry(){TOP::box().entry(STATE_AA);
assert(StateA::box().data == 42);
StateA::box().data = 43;
}
void exit() {
  TOP::box().exit(STATE_AA);
  assert(StateA::box().data == 43);
  StateA::box().data = 42;
}
...
private:
  void entry() {
    TOP::box().entry(STATE_AAA);
    assert(StateA::box().data == 43);
  }
  void exit() {
    TOP::box().exit(STATE_AAA);
    assert(StateA::box().data == 43);
  }
  void init() { TOP::box().init(STATE_AAA); }

  // Trying to mess up type inference here
  void init(const long &i) {
    TOP::box().init(STATE_AAA);
    box().data = i;
  }
...
private:
  void entry() {
    TOP::box().entry(STATE_AAB);
    assert(StateA::box().data == 43);
  }
  void exit() {
    TOP::box().exit(STATE_AAB);
    assert(StateA::box().data == 43);
  }
  void init() { TOP::box().init(STATE_AAB); }
};

SUBSTATE(StateAB, StateA){STATE(StateAB)

                              private : void entry(){TOP::box().entry(STATE_AB);
assert(StateA::box().data == 42);
}
void exit() {
  TOP::box().exit(STATE_AB);
  assert(StateA::box().data == 42);
}
...
SUBSTATE(StateABA, StateAB){STATE(StateABA)

                                private :
                                    void entry(){TOP::box().entry(STATE_ABA);
assert(StateA::box().data == 42);
}
void exit() {
  TOP::box().exit(STATE_ABA);
  assert(StateA::box().data == 42);
}
...
SUBSTATE(StateABB, StateAB){STATE(StateABB)

                                private :
                                    void entry(){TOP::box().entry(STATE_ABB);
assert(StateA::box().data == 42);
}
void exit() {
  TOP::box().exit(STATE_ABB);
  assert(StateA::box().data == 42);
}
...
SUBSTATE(StateB, Top){STATE(StateB) HISTORY()

                          private : void entry(){TOP::box().entry(STATE_B);
}
...
SUBSTATE(StateBA, StateB){STATE(StateBA)

                              private : void entry(){TOP::box().entry(STATE_BA);
}
...
SUBSTATE(StateBAA, StateBA){STATE(StateBAA)
...
SUBSTATE(StateBAB, StateBA){STATE(StateBAB)
...
SUBSTATE(StateBB, StateB){STATE(StateBB)

                              private : void entry(){TOP::box().entry(STATE_BB);
}
...
SUBSTATE(StateBBA, StateBB){STATE(StateBBA)
...
SUBSTATE(StateBBB, StateBB){STATE(StateBBB)
...
private:
  ...
  // Trying to mess up type inference here
  void init(const unsigned long &i, int = 0) {
    TOP::box().init(STATE_CAA);
    box().data = i;
  }
  void init(const char *, int = 0) {
    TOP::box().init(STATE_CAA);
    box().data = 0;
  }
};

SUBSTATE(StateCAB, StateCA){STATE(StateCAB)
...
SUBSTATE(StateCB, StateC){STATE(StateCB)

                              private : void entry(){TOP::box().entry(STATE_CB);
}
...
SUBSTATE(StateCBA, StateCB){STATE(StateCBA)
...
SUBSTATE(StateCBB, StateCB){STATE(StateCBB)
...
private:
  ...
  void init(const Macho::Alias &s) {
    TOP::box().init(STATE_X);
    box().state = s;
  }
};
...
enum { EVENT1, EVENT2, EVENT3, STATEA_ENTRY, STATEB_ENTRY };
...
private:
  void entry() { Top::box().push_back(STATEA_ENTRY); }
};
...
private:
  void entry() { Top::box().push_back(STATEB_ENTRY); }
};
...
template <class T> TSUBSTATE(TTop, T){TSTATE(TTop) HISTORY()};

template <class T> TSUBSTATE(TState1, TTop<T>) {
  TSTATE(TState1)

  virtual void event2();
};

template <class T> TSUBSTATE(TState2, TTop<T>) {
  TSTATE(TState2)

  virtual void event2();
};

template <class T> TSUBSTATE(TState3, TTop<T>) {
  TSTATE(TState3)
...
template <class T> void TState1<T>::event2() {
  setState<TState2<typename T::Other>>();
}

template <class T> void TState2<T>::event2() {
  setState<TState1<typename T::Other>>();
}

template <class T> void TState3<T>::event1() {
  setState<TState3<Macho::Anchor<StateA, 1>>>();
}

template <class T> void TState3<T>::event2() {
  setState<TState3<Macho::Anchor<StateA, 0>>>();
}
...
public:
  ...
  static void setState(Macho::Machine<typename T::TOP> &m, P1 p1) {
    m.setState(T::_getInstance(m), new Macho::_InitializerMain<T, P1>(p1));
  }
...
void testTransitions() {
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    TestAccess::setState<StateAAA>(m);
    // Increment data of persistent box: restoring the machine later should
...
void testDispatch() {
  ...
  assert(m.box()[0] == EVENT1);
  assert(m.box()[1] == EVENT2);
  assert(m.box()[2] == EVENT3);
  m->clear();
  ...
  m.dispatch(Event(&Top::event3, 3, true));
  assert(m.box()[0] == EVENT3);
  assert(m.box()[1] == STATEB_ENTRY);
  assert(m.box()[2] == EVENT1);

  m->clear();
  // Test if internal dispatch happens after switching to new state
  m.dispatch(Event(&Top::event3, 3, true));
  assert(m.box()[0] == EVENT3);
  assert(m.box()[1] == STATEA_ENTRY);
  assert(m.box()[2] == EVENT1);
}
